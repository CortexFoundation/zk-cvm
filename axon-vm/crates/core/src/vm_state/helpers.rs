use zkvm_opcodes::UNMAPPED_PAGE;
use zkvm_primitives::{
    aux::{MemoryKey, MemoryLocation},
...
>(
    ...
    let MemoryKey { location, timestamp } = key;
...
> VmState<S, M, EV, PP, DP, WT, N, E>
{
    ...
    pub fn read_code(&mut self, monotonic_cycle_counter: u32, key: MemoryKey) -> MemoryQuery {
        read_code(&self.memory, &mut self.witness_tracer, monotonic_cycle_counter, key)
    }
    ...
    pub fn write_memory(
        ...
        let partial_query =
            MemoryQuery { timestamp, location, value, value_is_pointer: is_pointer, rw_flag: true };
    ...
    pub fn call_precompile(&mut self, monotonic_cycle_counter: u32, query: LogQuery) {
        ...
        debug_assert_eq!(query.timestamp, self.timestamp_for_first_decommit_or_precompile_read());
        debug_assert!(!query.rw_flag);
        // add to witness
        self.witness_tracer
            .add_log_query(monotonic_cycle_counter, query);
    ...
    pub fn finish_frame(
        ...
        self.local_state.callstack.pop_entry()
    }
    ...
    pub fn perform_dst0_update(
        ...
        if let Some(location) = location {
            let key = MemoryKey { location, timestamp: self.timestamp_for_dst_write() };
    ...
    pub fn push_bootloader_context(
        ...
        assert!(!uf, "trying to create bootloader frame with more ergs than VM has available");
...
use zkvm_opcodes::{bitflags::bitflags, FatPointer};
