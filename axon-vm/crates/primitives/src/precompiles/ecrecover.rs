...
impl<const B: bool> Precompile for ECRecoverPrecompile<B> {
    ...
    fn execute_precompile<M: Memory>(
        ...
        let mut current_read_location = MemoryLocation {
            memory_type: MemoryType::Heap, /* we default for some value, here it's not that
                                            * important */
        ...
        let mut read_history = if B { Vec::with_capacity(MEMORY_READS_PER_CYCLE) } else { vec![] };
        ...
        let hash = hash_value.to_be_bytes();

        let r_bytes = r_value.to_be_bytes();

        let s_bytes = s_value.to_be_bytes();

        let v_bytes: [u8; 32] = v_value.to_be_bytes();
        let v = v_bytes[31];
        assert!(v == 0 || v == 1);
        ...
        if let Ok(recovered_pubkey) = pk {
            ...
            let ok_marker = U256::from_limbs([1, 0, 0, 0]);
            ...
            let result = U256::from_be_slice(&address);
        ...
        if B { Some((read_history, write_history, vec![round_witness])) } else { None }
    }
}
...
pub fn ecrecover_function<M: Memory, const B: bool>(
    ...
    memory: &mut M,
) -> Option<(Vec<MemoryQuery>, Vec<MemoryQuery>, Vec<ECRecoverRoundWitness>)> {
