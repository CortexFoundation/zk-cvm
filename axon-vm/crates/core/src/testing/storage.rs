...
impl InMemoryStorage {
    ...
    pub fn flatten_and_net_history(
        ...
        assert_eq!(self.frames_stack.len(), 1, "there must exist an initial keeper frame");
        ...
        let ApplicationData { forward, rollbacks: _ } = full_history;
        ...
        for el in forward.into_iter() {
            let LogQuery { timestamp, shard_id, address, key, rollback, .. } = &el;

            let entry = tmp.entry((*shard_id, *address, *key)).or_default();
    ...
    fn finish_frame(&mut self, _timestamp: Timestamp, panicked: bool) {
        ...
        if panicked {
            ...
            for query in rollbacks.iter().rev() {
                let LogQuery { shard_id, address, key, read_value, written_value, .. } = *query;
                let shard_level_map = &mut self.inner[shard_id as usize];
