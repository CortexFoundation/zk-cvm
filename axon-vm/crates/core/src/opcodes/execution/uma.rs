use zkvm_opcodes::{FatPointer, Opcode, UMAOpcode, UMA_INCREMENT_FLAG_IDX};
use zkvm_primitives::{aux::*, vm::MemoryType};

use super::*;

const U64_TOP_32_BITS_MASK: u64 = 0xffff_ffff_0000_0000;

use zkvm_opcodes::bitflags::bitflags;
...
impl<const N: usize, E: VmEncodingMode<N>> DecodedOpcode<N, E> {
    ...
    >(
        ...
        debug_assert!(dst0_mem_location.is_none(), "UMA opcode has dst0 not in register");
        ...
        let PrimitiveValue { value: src0_value, is_pointer: src0_is_ptr } = src0;
        ...
        // heap and aux heap are under full user's control, but fat pointers must be accessed in a
        // valid manner
        if is_ptr_read && !src0_is_ptr {
        ...
        let src_offset = if is_ptr_read {
            ...
            if src0.value > zkvm_opcodes::uma::MAX_OFFSET_TO_DEREF {
        ...
        let set_panic = !exceptions.is_empty();
        let legitimate_skip_memory_access = !skip_memory_access_flags.is_empty();
        ...
        // even if src_offset is beyond addressable memory, we are still fine with it as we will
        // eventually NOT do the read. When memory growth payments will be implemented we
        // will be able to remove restriction of 2^24 addressable memory
        ...
        let key_0 = MemoryKey { location: word_0_location, timestamp: timestamp_to_read };

        let word_0_read_value = if !skip_memory_access {
            let word_0_query =
        ...
        let word_1_read_value = if is_unaligned && !skip_memory_access {
            let key_1 = MemoryKey { location: word_1_location, timestamp: timestamp_to_read };
        ...
        match inner_variant {
            ...
            | a @ UMAOpcode::FatPointerRead => {
                ...
                // we want lowest bits of the word0 (if unalignment is 0 then we indeed want it in
                // full)
                ...
                result |= word_1_read_value >> ((32 - unalignment) * 8);
                ...
                if a == UMAOpcode::FatPointerRead {
                    ...
                    if uf || skip_memory_access {
                        // we either failed some validation above, so we don't care about the
                        // result, or the tail is in bounds too. If
                        // `incremented_offset` == `fat_ptr.length` there is no underflow,
                        // but the difference is 0 anyway
                        bytes_beyond_the_bound = 0;
                    }

                    bytes_beyond_the_bound %= 32;
                ...
                if !set_panic {
                    ...
                    if increment_offset {
                        let mut value_limbs = src0_value.into_limbs();
                        value_limbs[0] =
                            (value_limbs[0] & U64_TOP_32_BITS_MASK) + (increment_offset as u64);
            ...
            UMAOpcode::HeapWrite | UMAOpcode::AuxHeapWrite => {
                // we need to keep highest bytes of old word and place highest bytes of src1 into
                // lowest cleanup lowest bytes
                ...
                new_word_0_value |= src1 >> (unalignment * 8);
                ...
                new_word_1_value |= src1 << ((32 - word_1_highest_bytes) * 8);

                let key_0 = MemoryKey { location: word_0_location, timestamp: timestamp_to_write };

                let new_word_0_value =
                    PrimitiveValue { value: new_word_0_value, is_pointer: false };

                if !skip_memory_access {
                    // just write word 0
                ...
                // may be write word 1
                if is_unaligned && !skip_memory_access {
                    let key_1 =
                        MemoryKey { location: word_1_location, timestamp: timestamp_to_write };

                    // always not a pointer
                    let new_word_1_value =
                        PrimitiveValue { value: new_word_1_value, is_pointer: false };
                ...
                if !set_panic {
                    if increment_offset {
                        let mut value_limbs = src0_value.into_limbs();
                        value_limbs[0] =
                            (value_limbs[0] & U64_TOP_32_BITS_MASK) + (incremented_offset as u64);

                        let result = PrimitiveValue {
                            value: U256::from_limbs(value_limbs),
                            is_pointer: false,
                        };
                        debug_assert!(!src0_is_ptr);
