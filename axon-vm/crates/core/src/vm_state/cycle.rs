use tracing::*;
use zkvm_opcodes::{ImmMemHandlerFlags, NopOpcode, Operand, RegOrImmFlags};
...
>(
    ...
    if DT::CALL_BEFORE_DECODING {
        let local_state = VmLocalStateData { vm_local_state: local_state };
    ...
    let opcode_encoding = if !execution_has_ended && !pending_exception {
        let raw_opcode_u64 = match (code_pages_are_different, previous_super_pc == super_pc) {
            (true, _) | (false, false) => {
                ...
                let key =
                    MemoryKey { timestamp: local_state.timestamp_for_code_or_src_read(), location };

                delayed_changes.new_previous_code_memory_page = Some(code_page);

                // code read is never pending
                let code_query =
                    read_code(memory, witness_tracer, local_state.monotonic_cycle_counter, key);
    ...
    // now we have some candidate for opcode. If it's noop we are not expecting to have any
    // problems, so check for other meaningful exceptions
    ...
    let mut partially_decoded = DecodedOpcode { inner: partially_decoded_inner };

    if partially_decoded.variant.is_explicit_panic() {
        error_flags.set(ErrorFlags::INVALID_OPCODE, true);
    }

    // now try to get ergs price (unmodified for hard cases), that will also allow us to catch
    // invalid opcode
    let mut ergs_cost = zkvm_opcodes::OPCODES_PRICES[opcode_raw_variant_idx.into_usize()];
    if skip_cycle {
    ...
    let mask_into_panic_due_to_exception = !error_flags.is_empty();
    ...
    let resolved_condition = {
        ...
        match partially_decoded.condition {
            ...
            Condition::Ne => !local_state.flags.equality_flag,
            Condition::GtOrLt => {
...
> VmState<S, M, EV, PP, DP, WT, N, E>
{
    ...
    pub fn super_and_sub_pc_from_pc(pc: u16) -> (u16, u8) {
        ((pc >> OPCODES_PER_WORD_LOG_2), (pc & (OPCODES_PER_WORD as u16 - 1)) as u8)
    }
    ...
    pub fn cycle<DT: tracing::Tracer<N, E, SupportedMemory = M>>(
        ...
        let (after_masking_decoded, delayed_changes, skip_cycle) =
            read_and_decode(&self.local_state, &self.memory, &mut self.witness_tracer, tracer);
        delayed_changes.apply(&mut self.local_state);

        // now we are exception-less!

        // Now we can try to access the memory, but should keep in mind that if we did encounter an
        // error here then in addition to (later on) masking the opcode to be panicing Ret
        // we also have to avoid memory reads here and later on also avoid writes (this will
        // happen automatically inside of opcode processing itself)

        let mut mem_processor =
            MemOpsProcessor::<N, E> { sp: self.local_state.callstack.get_current_stack().sp };
        ...
        // here we can either execute NOP r0, r0, r0, r0 after masking, or RET (that doesn't
        // manipulate SP), so we can still set ergs and SP and be happy
        self.local_state.callstack.get_current_stack_mut().sp = mem_processor.sp;
        if after_masking_decoded.variant.opcode == zkvm_opcodes::Opcode::Nop(NopOpcode) {
        ...
        let src0_mem_value = if let Some(src0_mem_location) = src0_mem_location {
            ...
            PrimitiveValue { value: u256_word, is_pointer }
        } else {
        ...
        let (mut src0, mut src1) =
            if after_masking_decoded.variant.swap_operands() { (src1, src0) } else { (src0, src1) };
        ...
        if DT::CALL_BEFORE_EXECUTION {
            let local_state = VmLocalStateData { vm_local_state: &self.local_state };
        ...
        let prestate = PreState { src0, src1, dst0_mem_location, new_pc, is_kernel_mode };
        ...
        if DT::CALL_AFTER_EXECUTION {
            ...
            let data = AfterExecutionData { opcode: after_masking_decoded, dst0_mem_location };
