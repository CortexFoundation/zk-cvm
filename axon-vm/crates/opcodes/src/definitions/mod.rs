...
pub use self::{
    abi::*, add::*, all::*, binop::*, condition::*, context::*, div::*, far_call::*,
    invalid_opcode::*, jump::*, log::*, mul::*, near_call::*, noop::*, ptr::*, ret::*, shift::*,
    sub::*, uma::*, versioned_hash::*,
};
...
pub(crate) fn all_opcodes() -> Vec<Box<dyn OpcodeProps + 'static>> {
    ...
    assert_eq!(opcodes[0].name(), "Invalid opcode", "first opcode should be invalid opcode");
...
impl OpcodeVariant {
    ...
    pub fn is_explicit_panic(&self) -> bool {
        matches!(&self.opcode, Opcode::Invalid(_))
    }
}
...
impl std::fmt::Display for OpcodeVariant {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(f, "Non-exclusive flags: {:#01}|{:#01}", self.flags[0], self.flags[1],)
    }
}
...
pub fn synthesize_opcode_decoding_tables(
    ...
    for (i, opcode) in all_opcodes.iter().enumerate() {
        ...
        if !is_new {
            panic!("Duplicate opcode with name: {} at index {}", opcode.name(), i);
        }
    }
...
fn all_variants_in_version(version: ISAVersion) -> Vec<OpcodeVariant> {
    ...
    for (i, opcode) in all_opcodes.iter().enumerate() {
        if i == 0 {
            assert_eq!(opcode.name(), "Invalid opcode", "first opcode should be invalid opcode");
        }
        ...
        for variant_data in all_variants_in_this_version.into_iter() {
            ...
            match (
                ...
                | (&[Operand::Full(_), Operand::RegOnly], &[Operand::Full(_), Operand::RegOnly]) => {
                    // we need to encode all combinations of input and output being
                    // reg/stack/code/imm
                    let variant_idx = variant_data.variant_idx;
                    let num_non_exclusive_flags = variant_data.num_non_exclusive_flags;
                ...
                (&[Operand::Full(_)], &[]) => {
                    // we need to encode all combinations of input and output being
                    // reg/stack/code/imm but we only have a case of inputs
                    let variant_idx = variant_data.variant_idx;
                    let num_non_exclusive_flags = variant_data.num_non_exclusive_flags;
                    for input_variant in ImmMemHandlerFlags::all_variants() {
                        let mut tmp = INVALID_OPCODE_VARIANT;
...
fn semantically_equal(a: &OpcodeVariant, b: &OpcodeVariant) -> bool {
    ...
    matches!(
        (a.src0_operand_type, b.src0_operand_type),
        (Operand::RegOnly, Operand::RegOrImm(RegOrImmFlags::UseRegOnly))
            | (Operand::RegOrImm(RegOrImmFlags::UseRegOnly), Operand::RegOnly)
    )
}
...
pub fn compute_decoding_format(assert_version: ISAVersion) -> String {
    ...
    format!(
        ...
        total_bits, num_opcodes, num_options, num_flags, input_flags, output_flags
    )
}
...
mod test {
    ...
    fn show_num_variants() {
        ...
        println!("Need to encode {} variants in version {:?}", all, ISAVersion(0));
    }
    ...
    fn decode_from_raw(raw: u64) -> OpcodeVariant {
        ...
        OPCODES_TABLE[properties_index as usize]
    }
