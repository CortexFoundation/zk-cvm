...
{
    const UMA_HEAP_READ_OPCODE: zkvm_opcodes::Opcode =
        zkvm_opcodes::Opcode::UMA(UMAOpcode::HeapRead);
    const UMA_HEAP_WRITE_OPCODE: zkvm_opcodes::Opcode =
        zkvm_opcodes::Opcode::UMA(UMAOpcode::HeapWrite);
    const UMA_AUX_HEAP_READ_OPCODE: zkvm_opcodes::Opcode =
        zkvm_opcodes::Opcode::UMA(UMAOpcode::AuxHeapRead);
    const UMA_AUX_HEAP_WRITE_OPCODE: zkvm_opcodes::Opcode =
        zkvm_opcodes::Opcode::UMA(UMAOpcode::AuxHeapWrite);
    const UMA_FAT_PTR_READ_OPCODE: zkvm_opcodes::Opcode =
        zkvm_opcodes::Opcode::UMA(UMAOpcode::FatPointerRead);
...
impl<F: SmallField> QuasiFatPtrInUMA<F> {
    pub(crate) fn parse_and_validate<CS: ConstraintSystem<F>>(
        ...
        debug_assert_eq!(
            zkvm_opcodes::uma::MAX_OFFSET_TO_DEREF_LOW_U32 + 32u32,
            u32::MAX
        );
