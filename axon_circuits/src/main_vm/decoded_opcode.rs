...
pub fn perform_initial_decoding<F: SmallField, CS: ConstraintSystem<F>>(
    ...
    let requires_kernel_mode = aux_bools[zkvm_opcodes::KERNER_MODE_FLAG_IDX];
    let can_be_used_in_static_context =
        aux_bools[zkvm_opcodes::CAN_BE_USED_IN_STATIC_CONTEXT_FLAG_IDX];
    let explicit_panic = aux_bools[zkvm_opcodes::EXPLICIT_PANIC_FLAG_IDX];
    ...
    let panic_encoding = *zkvm_opcodes::PANIC_BITSPREAD_U64;
    ...
    let nop_encoding = *zkvm_opcodes::NOP_BITSPREAD_U64;
    // mask out aux bits (those are 0, but do it just in case)
...
use zkvm_opcodes::TOTAL_AUX_BITS;
...
>(
    ...
    if <CS::Config as CSConfig>::WitnessConfig::EVALUATE_WITNESS {
        ...
        let value_fn = move |inputs: [F; 1]| {
            ...
            if crate::config::CIRCUIT_VERSOBE {
                let opcode = zkvm_opcodes::OPCODES_TABLE[variant as usize];
                dbg!(opcode);
                let condition = zkvm_opcodes::condition::Condition::materialize_variant(
                    condition as usize,
                );
                dbg!(condition);
            }
