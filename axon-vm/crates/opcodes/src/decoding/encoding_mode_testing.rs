use super::*;
use crate::{
    merge_u4, split_as_u4, Condition, OpcodeVariant, CONDITIONAL_BITS_SHIFT,
    CONDITIONAL_BITS_WIDTH, DST_REGS_SHIFT, NOP_OPCODE_VARIANT, OPCODES_TABLE, OPCODES_TABLE_WIDTH,
    PANIC_OPCODE_VARIANT, SRC_REGS_SHIFT,
};
...
impl EncodingModeTesting {
    ...
    const UNUSED_GAP_RAW_MASK: u128 = (1u128
        << (Self::VARIANT_AND_CONDITION_BITS - OPCODES_TABLE_WIDTH - CONDITIONAL_BITS_WIDTH))
        - 1;
    const UNUSED_BITS_MASK: u128 = Self::UNUSED_GAP_RAW_MASK << OPCODES_TABLE_WIDTH;

    pub fn variant_and_condition_from_u128_word(
        raw_value: u128,
    ) -> (OpcodeVariant, Condition, usize) {
        let raw_value = raw_value as u64; // upper part is not relevant here
...
impl VmEncodingMode<16> for EncodingModeTesting {
    ...
    fn nop_encoding() -> Self::IntegerRepresentation {
        ...
        let full_nop_opcode = DecodedOpcode::<16, Self> {
            variant,
    ...
    fn encode_as_integer(opcode: &DecodedOpcode<16, Self>) -> Self::IntegerRepresentation {
        ...
        encoding |= (condition as u128) << CONDITIONAL_BITS_SHIFT;

        // register indexes
        let src_registers_byte = merge_u4(opcode.src0_reg_idx, opcode.src1_reg_idx);
        encoding |= (src_registers_byte as u128) << SRC_REGS_SHIFT;
        let dst_registers_byte = merge_u4(opcode.dst0_reg_idx, opcode.dst1_reg_idx);
        encoding |= (dst_registers_byte as u128) << DST_REGS_SHIFT;

        encoding |= (opcode.imm_0 as u128) << Self::IMM0_SHIFT;
        encoding |= (opcode.imm_1 as u128) << Self::IMM1_SHIFT;
    ...
    fn integer_representaiton_from_u256(
        ...
        match index {
            0 => value_limbs[2] as u128 + ((value_limbs[3] as u128) << 64),
            1 => value_limbs[0] as u128 + ((value_limbs[1] as u128) << 64),
            _ => unreachable!(),
        }
    }
