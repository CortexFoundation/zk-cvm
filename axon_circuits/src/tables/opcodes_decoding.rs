...
use zkvm_opcodes::OPCODES_TABLE_WIDTH;
...
pub fn create_opcodes_decoding_and_pricing_table<F: SmallField>() -> LookupTable<F, 3> {
    ...
    let num_rows = zkvm_opcodes::OPCODES_TABLE.len();
    assert_eq!(num_rows, 1 << OPCODES_TABLE_WIDTH);

    for x in 0..num_rows {
        let opcode_as_integer = x as u64;
        let opcode_props_encoding = zkvm_opcodes::OPCODES_PROPS_INTEGER_BITMASKS[x];
        let price = zkvm_opcodes::OPCODES_PRICES[x];
