...
impl<F: SmallField> OpcodeBitmask<F> {
    ...
    pub fn from_full_mask(mask: [Boolean<F>; TOTAL_OPCODE_MEANINGFULL_DESCRIPTION_BITS]) -> Self {
        ...
        debug_assert_eq!(
            OPCODE_VARIANT_BITS,
            zkvm_opcodes::max_num_variants_for_version(SUPPORTED_ISA_VERSION)
        );
        debug_assert_eq!(
            OPCODE_FLAGS_BITS,
            zkvm_opcodes::max_num_flags_for_version(SUPPORTED_ISA_VERSION)
        );
        debug_assert_eq!(
            TOTAL_OPCODE_DESCRIPTION_BITS_FLATTENED,
            zkvm_opcodes::total_description_bits_rounded_for_version(SUPPORTED_ISA_VERSION)
        );
        debug_assert_eq!(
            TOTAL_OPCODE_MEANINGFULL_DESCRIPTION_BITS,
            zkvm_opcodes::total_description_bits_for_version(SUPPORTED_ISA_VERSION)
        );
