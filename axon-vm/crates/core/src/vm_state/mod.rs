use zkvm_opcodes::{
    decoding::{encoding_mode_production::EncodingModeProduction, AllowedPcOrImm, VmEncodingMode},
    ISAVersion,
};
use zkvm_primitives::aux::{MemoryPage, Timestamp};
...
pub use self::{cycle::*, execution_stack::*, helpers::*, mem_ops::*};

pub const SUPPORTED_ISA_VERSION: ISAVersion = ISAVersion(0);

const _: () = if SUPPORTED_ISA_VERSION.0 != zkvm_opcodes::DEFAULT_ISA_VERSION.0 {
    panic!()
};

use zkvm_opcodes::{STARTING_BASE_PAGE, STARTING_TIMESTAMP};
...
impl PrimitiveValue {
    pub const fn empty() -> Self {
        Self { value: U256::ZERO, is_pointer: false }
    }

    pub const fn from_value(value: U256) -> Self {
        Self { value, is_pointer: false }
    }
}
...
pub struct VmLocalState<const N: usize = 8, E: VmEncodingMode<N> = EncodingModeProduction> {
    ...
    pub registers: [PrimitiveValue; zkvm_opcodes::REGISTERS_COUNT],
    pub flags: Flags,
    pub timestamp: u32,
...
impl<const N: usize, E: VmEncodingMode<N>> VmLocalState<N, E> {
    pub fn empty_state() -> Self {
        Self {
            ...
            registers: [PrimitiveValue::empty(); zkvm_opcodes::REGISTERS_COUNT],
            flags: Flags::empty(),
...
> VmState<S, M, EV, PP, DP, WT, N, E>
{
    ...
    pub fn increment_timestamp_after_cycle(&mut self) {
        self.local_state.timestamp += zkvm_opcodes::TIME_DELTA_PER_CYCLE
    }
    ...
    pub fn increment_memory_pages_on_call(&mut self) {
        self.local_state.memory_page_counter += zkvm_opcodes::NEW_MEMORY_PAGES_PER_FAR_CALL;
    }
}
