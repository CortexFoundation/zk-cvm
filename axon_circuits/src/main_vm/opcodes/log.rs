...
use crate::{
    ...
    main_vm::{
        opcodes::log::log_query::LogQueryWitness,
...
{
    const STORAGE_READ_OPCODE: zkvm_opcodes::Opcode =
        zkvm_opcodes::Opcode::Log(LogOpcode::StorageRead);
    const STORAGE_WRITE_OPCODE: zkvm_opcodes::Opcode =
        zkvm_opcodes::Opcode::Log(LogOpcode::StorageWrite);
    const L1_MESSAGE_OPCODE: zkvm_opcodes::Opcode =
        zkvm_opcodes::Opcode::Log(LogOpcode::ToL1Message);
    const EVENT_OPCODE: zkvm_opcodes::Opcode =
        zkvm_opcodes::Opcode::Log(LogOpcode::Event);
    const PRECOMPILE_CALL_OPCODE: zkvm_opcodes::Opcode =
        zkvm_opcodes::Opcode::Log(LogOpcode::PrecompileCall);
    ...
    use zkvm_opcodes::system_params::{
    ...
    let aux_byte_variable = Num::linear_combination(
        ...
        &[
            (
                ...
                F::from_u64_unchecked(zkvm_opcodes::system_params::STORAGE_AUX_BYTE as u64),
            ),
            (
                is_event.get_variable(),
                F::from_u64_unchecked(zkvm_opcodes::system_params::EVENT_AUX_BYTE as u64),
            ),
            (
                is_l1_message.get_variable(),
                F::from_u64_unchecked(zkvm_opcodes::system_params::L1_MESSAGE_AUX_BYTE as u64),
            ),
            (
                is_precompile.get_variable(),
                F::from_u64_unchecked(zkvm_opcodes::system_params::PRECOMPILE_AUX_BYTE as u64),
            ),
        ],
    )
    .get_variable();
