...
use zkvm_opcodes::PrecompileCallABI;
use zkvm_primitives::{
    auxiliary::*,
    queries::MemoryQuery,
    vm::{Memory, MemoryType, PrecompilesProcessor},
};
...
fn bytes_to_u256_words(input: Vec<u8>, unalignement: usize) -> Vec<U256> {
    let mut result = vec![];
    let mut it = std::iter::repeat(0xffu8).take(unalignement).chain(input);
    'outer: loop {
        let mut done = false;
        ...
        let el = U256::from_be_bytes(buffer);
        result.push(el);
        if done {
            break 'outer;
        }
...
fn pad_and_fill_memory<M: Memory>(
    ...
    for (index, word) in words.into_iter().enumerate() {
        let location = MemoryLocation {
            ...
            index: MemoryIndex(index as u32),
...
fn run_keccak256_test_inner(
    ...
    memory
        .heaps
        .push(((input_memory_page, vec![U256::ZERO; 1 << 10]), (0, vec![U256::ZERO; 0])));
    memory
        .page_numbers_indirections
        .insert(input_memory_page, reference_impls::memory::Indirection::Heap(1));
    ...
    let address = *zkvm_opcodes::system_params::KECCAK256_ROUND_FUNCTION_PRECOMPILE_FORMAL_ADDRESS;

    let precompile_query = LogQuery {
        timestamp: Timestamp(1),
        tx_number_in_block: 0,
        shard_id: 0,
        aux_byte: zkvm_opcodes::system_params::PRECOMPILE_AUX_BYTE,
        address,
