use zkvm_primitives::{aux::*, queries::*, vm::*};
...
impl<const B: bool> Default for SimpleDecommitter<B> {
    fn default() -> Self {
        Self::new()
    }
}
...
impl<const B: bool> SimpleDecommitter<B> {
    pub fn new() -> Self {
        Self { known_hashes: HashMap::default(), history: HashMap::default() }
    }
...
impl<const B: bool> DecommittmentProcessor for SimpleDecommitter<B> {
    fn decommit_into_memory<M: Memory>(
        ...
        if let Some((old_page, old_len)) = self.history.get(&partial_query.hash).copied() {
            ...
            self.history.insert(
                partial_query.hash,
                (partial_query.memory_page.0, partial_query.decommitted_length),
            );
            if B {
