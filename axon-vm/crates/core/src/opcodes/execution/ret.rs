use zkvm_opcodes::{
    definitions::ret::*, FatPointer, FatPointerValidationException, Opcode, RetABI,
    RetForwardPageType, RetOpcode,
};
use zkvm_primitives::aux::Timestamp;
...
impl<const N: usize, E: VmEncodingMode<N>> DecodedOpcode<N, E> {
    ...
    >(
        ...
        let PrimitiveValue { value: mut src0, is_pointer: mut src0_is_ptr } = src0;
        ...
        let RetABI { mut memory_quasi_fat_pointer, page_forwarding_mode } = ret_abi;
        ...
        if !current_callstack.is_local_frame {
            ...
            if page_forwarding_mode == RetForwardPageType::ForwardFatPointer {
                ...
                if memory_quasi_fat_pointer.memory_page < current_callstack.base_memory_page.0 {
                    ...
                    // - caller modifies calldata corresponding heap region, that leads to
                    //   modification of returndata
            ...
            // our formal definition of "in bounds" is strictly "less than", but we want to allow to
            // return "trivial" pointer, like `ret.ok r0`
            // this captures the case of empty slice
            if !memory_quasi_fat_pointer.validate_as_slice() {
                inner_variant = RetOpcode::Panic;
            }
        ...
        let fat_ptr_for_returndata = if current_callstack.is_local_frame {
            None
        } else {
            match inner_variant {
            ...
            // we do nothing with it later on, so just keep returndata page, and set zeroes for
            // other
        ...
        if !finished_callstack.is_local_frame {
            ...
            vm_state.local_state.registers[RET_IMPLICIT_RETURNDATA_PARAMS_REGISTER as usize] =
                PrimitiveValue { value: returndata_fat_pointer.to_u256(), is_pointer: true };
