use zkvm_opcodes::{INITIAL_SP_ON_FAR_CALL, UNMAPPED_PAGE};
...
impl<const N: usize, E: VmEncodingMode<N>> CallStackEntry<N, E> {
    ...
    pub fn empty_context() -> Self {
        Self {
            ...
            ergs_remaining: zkvm_opcodes::system_params::VM_INITIAL_FRAME_ERGS,
            this_shard_id: 0u8,
            caller_shard_id: 0u8,
            code_shard_id: 0u8,
            is_static: false,
    ...
    pub const fn get_address_low(&self) -> u16 {
        let address_bytes = self.this_address.into_array();

        u16::from_le_bytes([address_bytes[19], address_bytes[18]])
    }
...
impl<const N: usize, E: VmEncodingMode<N>> Callstack<N, E> {
    pub fn empty() -> Self {
        let new =
            Self { current: CallStackEntry::empty_context(), inner: Vec::with_capacity(1 << 12) };
    ...
    pub fn push_entry(&mut self, entry: CallStackEntry<N, E>) {
        ...
        self.inner.push(old);
        debug_assert!(self.depth() <= zkvm_opcodes::system_params::VM_MAX_STACK_DEPTH as usize);
    }
    ...
    pub fn is_full(&self) -> bool {
        self.depth() == zkvm_opcodes::system_params::VM_MAX_STACK_DEPTH as usize
    }
