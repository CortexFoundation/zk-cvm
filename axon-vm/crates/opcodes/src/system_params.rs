...
use crate::{
    circuit_prices::STORAGE_WRITE_HASHER_MIN_COST_IN_ERGS, u64_to_address, CALL_LIKE_ERGS_COST,
};
...
pub const SHA256_ROUND_FUNCTION_PRECOMPILE_ADDRESS: u16 = 0x02; // as in Cortex
pub const ECRECOVER_INNER_FUNCTION_PRECOMPILE_ADDRESS: u16 = 0x01; // as in Cortex
...
/// 128k * 4 / 32 -- the maximal realistic smart contract size. NOTE this constant should be updated
/// once a new packing method is introduced or more than 128k of data is allowed to be sent on L1.
...
/// The minimum amount of ergs that should be spent by the user while using the MsgValueSimulator
/// (even if the user spends less funds, only the parent frame will receivet the refund)
...
/// The minimum price in ergs that a storage write should cost in order to protect Ethereum's
/// `.transfer / .send` function against reentrancy. The first part of the expression is the stipend
/// given by the MsgValueSimulator to the callee frame. The second part of the expression
/// is the 2300 constant used for 0-value `transfer/send` calls + 1 to make sure that within the
/// call it is not possible to store anything.
pub const MIN_STORAGE_WRITE_PRICE_FOR_REENTRANCY_PROTECTION: u32 =
    max(MSG_VALUE_SIMULATOR_ADDITIVE_COST - MSG_VALUE_SIMULATOR_MIN_USED_ERGS + 1, 2300 + 1);

/// The minimal price in ergs the storage could cost to protect against reentrancy + take into
/// account the usage of the single instance circuits.
pub const MIN_STORAGE_WRITE_COST: u32 =
    max(MIN_STORAGE_WRITE_PRICE_FOR_REENTRANCY_PROTECTION, STORAGE_WRITE_HASHER_MIN_COST_IN_ERGS);

pub static BOOTLOADER_FORMAL_ADDRESS: Lazy<Address> =
    Lazy::new(|| u64_to_address(BOOTLOADER_FORMAL_ADDRESS_LOW as u64));
pub static DEPLOYER_SYSTEM_CONTRACT_ADDRESS: Lazy<Address> =
    Lazy::new(|| u64_to_address(DEPLOYER_SYSTEM_CONTRACT_ADDRESS_LOW as u64));
pub static NONCE_MANAGER_SYSTEM_CONTRACT_ADDRESS: Lazy<Address> =
    Lazy::new(|| u64_to_address(SYSTEM_CONTRACTS_OFFSET_ADDRESS as u64 + 0x03));
pub static KNOWN_CODE_FACTORY_SYSTEM_CONTRACT_ADDRESS: Lazy<Address> =
    Lazy::new(|| u64_to_address(SYSTEM_CONTRACTS_OFFSET_ADDRESS as u64 + 0x04));
pub static KECCAK256_ROUND_FUNCTION_PRECOMPILE_FORMAL_ADDRESS: Lazy<Address> =
    Lazy::new(|| u64_to_address(KECCAK256_ROUND_FUNCTION_PRECOMPILE_ADDRESS as u64));
pub static SHA256_ROUND_FUNCTION_PRECOMPILE_FORMAL_ADDRESS: Lazy<Address> =
    Lazy::new(|| u64_to_address(SHA256_ROUND_FUNCTION_PRECOMPILE_ADDRESS as u64));
pub static ECRECOVER_INNER_FUNCTION_PRECOMPILE_FORMAL_ADDRESS: Lazy<Address> =
    Lazy::new(|| u64_to_address(ECRECOVER_INNER_FUNCTION_PRECOMPILE_ADDRESS as u64));
