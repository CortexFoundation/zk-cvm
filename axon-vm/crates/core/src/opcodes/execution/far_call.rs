use zkvm_opcodes::{
    bitflags::bitflags,
    definitions::far_call::*,
    system_params::{DEPLOYER_SYSTEM_CONTRACT_ADDRESS, STORAGE_AUX_BYTE},
    FatPointerValidationException, INITIAL_SP_ON_FAR_CALL, UNMAPPED_PAGE,
};
use zkvm_primitives::{aux::*, queries::LogQuery};
...
use zkvm_opcodes::{FarCallABI, FarCallForwardPageType, FarCallOpcode, FatPointer, Opcode};
...
impl<const N: usize, E: VmEncodingMode<N>> DecodedOpcode<N, E> {
    ...
    >(
        ...
        let PreState { src0, src1, new_pc, is_kernel_mode, .. } = prestate;
        let inner_variant = match self.variant.opcode {
            Opcode::FarCall(inner) => inner,
            _ => unreachable!(),
        };

        let PrimitiveValue { value: abi_src, is_pointer: abi_src_is_ptr } = src0;
        let PrimitiveValue { value: call_destination_value, is_pointer: _ } = src1;
        ...
        far_call_abi.constructor_call &= is_kernel_mode;
        far_call_abi.to_system &= dst_is_kernel;
        ...
        let new_code_shard_id = if is_call_shard { far_call_abi.shard_id } else { caller_shard_id };
        ...
        let (mapped_code_page, ergs_after_code_read_and_exceptions_resolution, stipend_for_callee) = {
            let (code_hash, map_to_trivial) = if new_code_shard_id != 0 {
                ...
                // mask for default AA
                let mask_into_default_aa = code_hash_from_storage.is_zero() && !dst_is_kernel;
            ...
            let memory_page_candidate_for_code_decommittment = if map_to_trivial {
            ...
            use zkvm_opcodes::{ContractCodeSha256, VersionedHashGeneric};

            let buffer = code_hash.to_be_bytes();
                ...
                if !code_marker_is_valid {
                    ...
                    if can_call_code_without_masking {
                        ...
                        if !dst_is_kernel {
                            ...
                            let buffer =
                                vm_state.block_properties.default_aa_code_hash.to_be_bytes();
                            let versioned_hash =
            ...
            if far_call_abi.forwarding_mode == FarCallForwardPageType::ForwardFatPointer
                && !abi_src_is_ptr
            {
                exceptions.set(FarCallExceptionFlags::INPUT_IS_NOT_POINTER_WHEN_EXPECTED, true);
            }
            ...
            // NOTE: one can not properly address a range [2^32 - 32..2^32] here, but we never care
            // in practice about this case as one can not ever pay to grow memory to
            // such extent
            ...
            if !far_call_abi.memory_quasi_fat_pointer.validate_as_slice() {
                exceptions.set(FarCallExceptionFlags::MALFORMED_ABI_QUASI_POINTER, true);
            }

            // these modifications we can do already as all pointer formal validity related things
            // are done
            ...
            let memory_growth_in_bytes = match far_call_abi.forwarding_mode {
                a @ FarCallForwardPageType::UseHeap | a @ FarCallForwardPageType::UseAuxHeap => {
                    // pointer is already validated, so we do not need to check that start + length
                    // do not overflow
                    ...
                    if uf {
                        ...
                        // Note that we are ok so save even penalizing upper bound because we will
                        // burn all the ergs in this frame anyway, and no
                        // further resizes are possible
            ...
            let cost_of_memory_growth =
                memory_growth_in_bytes.wrapping_mul(zkvm_opcodes::MEMORY_GROWTH_ERGS_PER_BYTE);
            ...
            let mut msg_value_stipend = if !FORCED_ERGS_FOR_MSG_VALUE_SIMULATOR {
                0
            } else if called_address_as_u256 == U256::from(zkvm_opcodes::ADDRESS_MSG_VALUE as u64)
                && far_call_abi.to_system
            {
                // use that doesn't know what's doing is trying to call "transfer"

                let pubdata_related = vm_state
                    .local_state
                    .current_ergs_per_pubdata_byte
                    .checked_mul(
                        zkvm_opcodes::system_params::MSG_VALUE_SIMULATOR_PUBDATA_BYTES_TO_PREPAY,
                    )
                    .expect("must fit into u32");
                pubdata_related
                    .checked_add(zkvm_opcodes::system_params::MSG_VALUE_SIMULATOR_ADDITIVE_COST)
            ...
            let remaining_ergs_of_caller_frame = if remaining_ergs_after_growth >= msg_value_stipend
            {
                ...
                exceptions
                    .set(FarCallExceptionFlags::NOT_ENOUGH_ERGS_FOR_EXTRA_FAR_CALL_COSTS, true);
            ...
            let cost_of_decommittment =
                zkvm_opcodes::ERGS_PER_CODE_WORD_DECOMMITTMENT * code_length_in_words;
            ...
            let code_memory_page = if !exceptions.is_empty() {
                ...
                if !processed_decommittment_query.is_fresh {
                    // refund
        ...
        let (passed_ergs, remaining_ergs_for_this_context) = {
            ...
            if uf {
                ...
                (far_call_abi.ergs_passed, leftover + remaining_from_max_passable)
            }
        };
        ...
        let new_stack = CallStackEntry {
            ...
            heap_bound: zkvm_opcodes::system_params::NEW_FRAME_MEMORY_STIPEND,
            aux_heap_bound: zkvm_opcodes::system_params::NEW_FRAME_MEMORY_STIPEND,
        };
        ...
        let mut r2_value_limbs = [0, 0, 0, 0];
        if far_call_abi.constructor_call {
            r2_value_limbs[0] |= 1u64;
        }
        if far_call_abi.to_system {
            r2_value_limbs[0] |= 1u64 << 1;
        }

        vm_state.local_state.registers[CALL_IMPLICIT_CONSTRUCTOR_MARKER_REGISTER as usize] =
            PrimitiveValue { value: U256::from_limbs(r2_value_limbs), is_pointer: false };
