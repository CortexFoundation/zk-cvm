...
>(
    ...
    ctx.saved_context.base_page =
        UInt32::allocated_constant(cs, zkvm_opcodes::BOOTLOADER_BASE_PAGE);
    ctx.saved_context.code_page =
        UInt32::allocated_constant(cs, zkvm_opcodes::BOOTLOADER_CODE_PAGE);
    ...
    ctx.saved_context.exception_handler_loc = UInt16::allocated_constant(
        cs,
        zkvm_opcodes::system_params::INITIAL_FRAME_FORMAL_EH_LOCATION,
    );
    ctx.saved_context.ergs_remaining =
        UInt32::allocated_constant(cs, zkvm_opcodes::system_params::VM_INITIAL_FRAME_ERGS);

    let formal_bootloader_address_low = UInt32::allocated_constant(
        cs,
        zkvm_opcodes::system_params::BOOTLOADER_FORMAL_ADDRESS_LOW as u32,
    );
    ...
    ctx.saved_context.heap_upper_bound =
        UInt32::allocated_constant(cs, zkvm_opcodes::system_params::BOOTLOADER_MAX_MEMORY);
    ...
    bootloaded_state.timestamp =
        UInt32::allocated_constant(cs, zkvm_opcodes::STARTING_TIMESTAMP);
    bootloaded_state.memory_page_counter =
        UInt32::allocated_constant(cs, zkvm_opcodes::STARTING_BASE_PAGE);
    ...
    let formal_ptr = zkvm_opcodes::FatPointer {
        offset: 0,
        memory_page: zkvm_opcodes::BOOTLOADER_CALLDATA_PAGE,
        start: 0,
        length: 0,
    };
