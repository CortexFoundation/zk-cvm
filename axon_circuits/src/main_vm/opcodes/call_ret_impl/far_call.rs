...
use zkvm_opcodes::system_params::STORAGE_AUX_BYTE;
...
impl<F: SmallField> FarCallPartialABI<F> {
    pub fn from_register_view<CS: ConstraintSystem<F>>(
        ...
        let shard_id = input.u8x32_view
            [zkvm_opcodes::definitions::abi::far_call::FAR_CALL_SHARD_ID_BYTE_IDX];
        let constructor_call = input.u8x32_view
            [zkvm_opcodes::definitions::abi::far_call::FAR_CALL_CONSTRUCTOR_CALL_BYTE_IDX]
            .is_zero(cs)
            .negated(cs);
        let system_call = input.u8x32_view
            [zkvm_opcodes::definitions::abi::far_call::FAR_CALL_SYSTEM_CALL_BYTE_IDX]
            .is_zero(cs)
            .negated(cs);
    ...
    [(); <DecommitQuery<F> as CSAllocatableExt<F>>::INTERNAL_STRUCT_LEN]:,
{
    ...
    const FAR_CALL_OPCODE: zkvm_opcodes::Opcode =
        zkvm_opcodes::Opcode::FarCall(zkvm_opcodes::FarCallOpcode::Normal);
    ...
    let _is_normal_call = common_opcode_state
        .decoded_opcode
        .properties_bits
        .boolean_for_variant(zkvm_opcodes::Opcode::FarCall(
            zkvm_opcodes::FarCallOpcode::Normal,
        ));
    let is_delegated_call = common_opcode_state
        .decoded_opcode
        .properties_bits
        .boolean_for_variant(zkvm_opcodes::Opcode::FarCall(
            zkvm_opcodes::FarCallOpcode::Delegate,
        ));
    let is_mimic_call = common_opcode_state
        .decoded_opcode
        .properties_bits
        .boolean_for_variant(zkvm_opcodes::Opcode::FarCall(
            zkvm_opcodes::FarCallOpcode::Mimic,
        ));
    ...
    new_callstack_entry.heap_upper_bound = UInt32::allocated_constant(
        cs,
        zkvm_opcodes::system_params::NEW_FRAME_MEMORY_STIPEND,
    );
    ...
    let implicit_mimic_call_reg = draft_vm_state.registers
        [zkvm_opcodes::definitions::far_call::CALL_IMPLICIT_PARAMETER_REG_IDX as usize];
    ...
    let code_hash_version_byte =
        UInt8::allocated_constant(cs, zkvm_opcodes::ContractCodeSha256::VERSION_BYTE);
    ...
    let now_in_construction_marker_byte = UInt8::allocated_constant(
        cs,
        zkvm_opcodes::ContractCodeSha256::YET_CONSTRUCTED_MARKER,
    );
    ...
    let marker_byte_masked = UInt8::allocated_constant(
        cs,
        zkvm_opcodes::ContractCodeSha256::CODE_AT_REST_MARKER,
    );
    ...
    // if we call now-in-construction system contract, then we formally mask into 0 (even though
    // it's not needed), and we should put an exception here
    ...
    // exceptions, along with `bytecode_hash_is_trivial` indicate whether we will or will decommit
    // code into memory, or will just use UNMAPPED_PAGE
    ...
    let callee_stipend = if FORCED_ERGS_FOR_MSG_VALUE_SIMUALTOR == false {
        ...
        let is_msg_value_simulator_address_low =
            UInt32::allocated_constant(cs, zkvm_opcodes::ADDRESS_MSG_VALUE as u32);
        let target_low_is_msg_value_simulator = UInt32::equals(
            cs,
            &destination_address.inner[0],
            &is_msg_value_simulator_address_low,
        );
        // we know that that msg.value simulator is kernel, so we test equality of low address
        // segment and test for kernel
        ...
        let additive_cost = UInt32::allocated_constant(
            cs,
            zkvm_opcodes::system_params::MSG_VALUE_SIMULATOR_ADDITIVE_COST,
        );
    ...
    for reg_idx in zkvm_opcodes::definitions::far_call::CALL_SYSTEM_ABI_REGISTERS {
        register_zero_out[reg_idx as usize] = Some(cleanup_register);
    }
    for reg_idx in zkvm_opcodes::definitions::far_call::CALL_RESERVED_RANGE {
    ...
    [(); <LogQuery<F> as CSAllocatableExt<F>>::INTERNAL_STRUCT_LEN]:,
{
    ...
    let deployer_contract_address_low = UInt32::allocated_constant(
        cs,
        zkvm_opcodes::system_params::DEPLOYER_SYSTEM_CONTRACT_ADDRESS_LOW as u32,
    );
    ...
    [(); <DecommitQuery<F> as CSAllocatableExt<F>>::INTERNAL_STRUCT_LEN]:,
{
    ...
    let cost_of_decommit_per_word =
        UInt32::allocated_constant(cs, zkvm_opcodes::ERGS_PER_CODE_WORD_DECOMMITTMENT);
    ...
    // we use `should_decommit` as a marker that we did actually execute both read and decommittment
