...
// ALL traits here are for execution and NOT for witness generation. They can depend on one another,
...
// not to accept a transaction (or revert to the previous state) and actually perform "huge"
// rollbacks on on all the corresponding implementors
...
pub trait DecommittmentProcessor: std::fmt::Debug {
    // For calls to external contract we use storage read + request to decommit a particular hash
    // into some memory page. We also optimize in a way that since code and calldata locations
    // ar read-only we can just give already filled page if we decommit the same hash.
    // We also optinally return a set of memory writes that such decommitment has made (if it's a
    // new page) for witness generation at the end of the block
...
/// Abstraction over precompile implementation. Precompile is usually a closure-forming FSM, so it
/// must output some cycle-like witness
pub trait Precompile: std::fmt::Debug {
    ...
    /// - all memory writes (depending on the implementation we may directly write to `memory` and
    ///   also remove it)
