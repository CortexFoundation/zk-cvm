...
use zkvm_opcodes::{FatPointer, BOOTLOADER_CALLDATA_PAGE};
use zkvm_primitives::{
    aux::{MemoryPage, Timestamp},
    queries::MemoryQuery,
    vm::{Memory, MemoryType, MAX_CODE_PAGE_SIZE_IN_WORDS, MAX_STACK_PAGE_SIZE_IN_WORDS},
};

use super::*;
use crate::vm_state::{CallStackEntry, PrimitiveValue};

pub struct ReusablePool<T: Sized, InitFn: Fn() -> T, OnPullFn: Fn(&mut T), OnReturnFn: Fn(&mut T)> {
    pool: Vec<T>,
    init_fn: InitFn,
    on_pull_fn: OnPullFn,
    ...
    ReusablePool<T, InitFn, OnPullFn, OnReturnFn>
{
    pub fn new_with_capacity(
        ...
        Self { pool, init_fn, on_pull_fn, on_return_fn }
    }
...
pub fn new_reference_memory_impl()
-> ReusablePool<Vec<U256>, impl Fn() -> Vec<U256>, impl Fn(&mut Vec<U256>), impl Fn(&mut Vec<U256>)>
{
    ReusablePool::new_with_capacity(1 << 10, heap_init, heap_on_pull, heap_on_return)
}

impl<S: BuildHasher + Default> Default for SimpleMemory<S> {
    fn default() -> Self {
        Self::new()
    }
}
...
impl<S: BuildHasher + Default> SimpleMemory<S> {
    pub fn new() -> Self {
        let mut new = Self {
            stack_pages: Vec::with_capacity(1024), /* we do not need stack or heaps for root
                                                    * frame as it's never accessible */
        ...
        new.heaps
            .push(((0u32, vec![U256::ZERO; 1 << 10]), (0u32, vec![U256::ZERO; 1 << 20]))); // formally, so we can access "last"
...
impl Memory for SimpleMemory {
        ...
        match query.location.memory_type {
            MemoryType::Stack => {
                if query.rw_flag {
                    ...
                    let primitive =
                        PrimitiveValue { value: query.value, is_pointer: query.value_is_pointer };
                    assert!(
            ...
            MemoryType::FatPointer => {
                ...
                // was NEVER written into, so it's page would NOT be resized to the index which we
                // try to access (even though fat pointe IS in bounds), so we need
                // to use .get()
                match indirection {
    ...
    fn specialized_code_query(
        ...
        if query.rw_flag {
            self.code_pages
                .entry(page)
                .or_insert_with(|| vec![U256::ZERO; MAX_CODE_PAGE_SIZE_IN_WORDS]);
            let page_content = self.code_pages.get_mut(&page).unwrap();
            page_content[idx] = query.value;
        } else {
            debug_assert!(!query.value_is_pointer);
    ...
    fn start_global_frame(
        ...
        use zkvm_opcodes::decoding::EncodingModeProduction;
        ...
        // self.stack_pages.push((stack_page.0, vec![PrimitiveValue::empty();
        // MAX_STACK_PAGE_SIZE_IN_WORDS]));
        ...
        self.heaps
            .push(((heap_page.0, heap_page_from_pool), (aux_heap_page.0, aux_heap_page_from_pool)));
        ...
        if calldata_fat_pointer.memory_page == 0 {
            ...
            self.page_numbers_indirections
                .insert(current_heap_page, Indirection::Heap(idx_to_use_for_calldata_ptrs));
            // if we will return from here and returndata page will not "leak" calldata page via
            // forwarding, then we can cleanup the indirection
            self.indirections_to_cleanup_on_return
                .last_mut()
                .unwrap()
            ...
            self.page_numbers_indirections
                .insert(current_aux_heap_page, Indirection::AuxHeap(idx_to_use_for_calldata_ptrs));
            self.indirections_to_cleanup_on_return
                .last_mut()
                .unwrap()
            ...
            match existing_indirection {
                ...
                a => {
                    panic!(
                        "calldata forwaring using pointer {:?} should already have a heap/aux heap indirection, but has {:?}. All indirections:\n {:?}",
                        &calldata_fat_pointer, a, &self.page_numbers_indirections
                    );
                }
            }
        }
    ...
    fn finish_global_frame(
        ...
        // we can cleanup all the heap pages that derive from base, if one of those is not in
        // returndata
