use zkvm_opcodes::system_params::{EVENT_AUX_BYTE, L1_MESSAGE_AUX_BYTE};
use zkvm_primitives::{aux::Timestamp, queries::LogQuery, vm::EventSink};
...
impl<T> ApplicationData<T> {
    pub fn empty() -> Self {
        Self { forward: vec![], rollbacks: vec![] }
    }
}
