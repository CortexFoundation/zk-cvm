...
impl FatPointer {
    ...
    pub const fn empty() -> Self {
        Self { length: 0, start: 0, memory_page: 0, offset: 0 }
    }

    pub const fn from_u256(value: U256) -> Self {
        let raw_value = value.as_limbs();
        ...
        Self { offset, memory_page, start, length }
    }
    ...
    pub fn validate(self, is_fresh: bool) -> FatPointerValidationException {
        ...
        if is_fresh && self.offset != 0 {
            exceptions.set(FatPointerValidationException::OFFSET_IS_NOT_ZERO_WHEN_EXPECTED, true);
        }
    ...
    pub const fn to_u256(self) -> U256 {
        let mut result = U256::ZERO.into_limbs();
        result[0] = (self.offset as u64) | ((self.memory_page as u64) << 32);

        result[1] = (self.start as u64) | ((self.length as u64) << 32);
