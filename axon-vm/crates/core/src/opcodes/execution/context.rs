use zkvm_opcodes::{ContextOpcode, Opcode};
...
impl<const N: usize, E: VmEncodingMode<N>> DecodedOpcode<N, E> {
    ...
    >(
        ...
        let PreState { src0, new_pc, dst0_mem_location, .. } = prestate;
        ...
        if inner_variant == ContextOpcode::SetContextU128 {
            vm_state.local_state.context_u128_register = low_u128_of_u256(&src0);
            return;
        }

        if inner_variant == ContextOpcode::SetErgsPerPubdataByte {
            vm_state.local_state.current_ergs_per_pubdata_byte = low_u32_of_u256(&src0);
            return;
        }
        ...
        let value = match inner_variant {
            ...
            ContextOpcode::Meta => {
                use zkvm_opcodes::VmMetaParameters;
        ...
        let result = PrimitiveValue { value, is_pointer: false };
