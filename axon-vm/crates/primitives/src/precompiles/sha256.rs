...
// for sha256 we do not need complicated buffering as it uses 64 bytes per round, and this is
// divisible by our 32 byte per query
...
impl<const B: bool> Precompile for Sha256Precompile<B> {
    ...
    fn execute_precompile<M: Memory>(
        ...
        let mut read_queries =
            if B { Vec::with_capacity(MEMORY_READS_PER_CYCLE * num_rounds) } else { vec![] };

        let mut write_queries =
            if B { Vec::with_capacity(MEMORY_WRITES_PER_CYCLE) } else { vec![] };

        let mut witness = if B { Vec::with_capacity(num_rounds) } else { vec![] };
        ...
        for round in 0..num_rounds {
            ...
            for query_index in 0..MEMORY_READS_PER_CYCLE {
                ...
                // TODO(steins): Try to optimise the clone of the data bytes.
                block[(query_index * 32)..(query_index * 32 + 32)]
                    .copy_from_slice(&data.to_be_bytes::<32>());
            }
            ...
            let mut round_witness = Sha256RoundWitness { new_request: None, reads, writes: None };
            ...
            if is_last {
                ...
                let as_u256 = U256::from_be_bytes(hash_as_bytes32);

                let write_location = MemoryLocation {
                    memory_type: MemoryType::Heap, /* we default for some value, here it's not
                                                    * that important */
        ...
        if B { Some((read_queries, write_queries, witness)) } else { None }
    }
}
...
pub fn transmute_state(reference_state: Sha256) -> Sha256InnerState {
    // we use a trick that size of both structures is the same, and even though we do not know a
    // stable field layout, we can replicate it
